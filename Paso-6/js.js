...
/**Variables (Lets) */
let base_preguntas = readText("/json/paso-6.json");
...
function getPreguntasExisten() {
  if (preguntaIndex < interprete_bp.length) {
    ...
    setTimeout(() => {
      var url = "/Paso-6/html/desicion.html";
      window.location.href = url;
    }, 1000);
  }
}
...
function mostrarAlerta() {
  ...
  if (!checkbox.checked) {
    ...
    window.location = "/Paso-6/html/reflexion.html";
  }
}
...
function guardarPasoCompleto() {
  ...
  localStorage.setItem("pasoSeis", "true");
}
