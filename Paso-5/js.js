...
/**Variables (Lets) */
let base_preguntas = readText("/json/paso-5.json");
...
function getPreguntasExisten() {
  if (preguntaIndex < interprete_bp.length) {
    ...
    setTimeout(() => {
      var url = "/Paso-5/html/desicion.html";
      window.location.href = url;
    }, 1000);
  }
}
...
function mostrarAlerta() {
  ...
  if (!checkbox.checked) {
    ...
    window.location = "/Paso-5/html/reflexion.html";
  }
}
...
function guardarPasoCompleto() {
  ...
  localStorage.setItem("pasoCinco", "true");
}
