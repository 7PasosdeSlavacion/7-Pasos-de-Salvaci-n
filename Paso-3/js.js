...
/**Variables (Lets) */
let base_preguntas = readText("/json/paso-3.json");
...
function getPreguntasExisten() {
  if (preguntaIndex < interprete_bp.length) {
    ...
    setTimeout(() => {
      var url = "/Paso-3/html/desicion.html";
      window.location.href = url;
    }, 1000);
  }
}
...
function mostrarAlerta() {
  ...
  if (!checkbox.checked) {
    ...
    window.location = "/Paso-3/html/reflexion.html";
  }
}
...
function guardarPasoCompleto() {
  ...
  localStorage.setItem("pasoTres", "true");
}
